...
내가 먼저 생각한 원인:SELECT 명령어의 철자를 잘못 입력했을 수 있다고 생각했다.

실제로 확인한 방법:오류 메시지에 표시된 SELEC 부분을 확인하고, 올바른 SELECT 명령어와 철자를 비교했다.

실제 원인:SELECT에서 마지막 글자인 T를 빠뜨려 발생한 SQL 문법 오류였다.
...
```text
재검증 결과:SELECT 1;의 결과로 1이 출력되었고, SELECT current_database();의 결과로 ai_database_book이 출력되었다. SQL 문법과 데이터베이스 연결이 모두 정상임을 확인했다.
```
...
```text
SELECT를 SELEC로 잘못 입력해서 발생한 오류였으며, 철자를 수정한 뒤 정상적으로 실행되었기 때문에 SQL 문법 문제로 분류했다.
```
...
```text
DBeaver에서 PostgreSQL을 사용하고 있습니다. SELEC 1;을 실행했더니 syntax error at or near "SELEC"라는 오류가 발생했습니다. 오류의 원인과 안전하게 확인하고 수정하는 방법을 알려주세요.
```
...
| --- | --- | --- | --- |
| 오류 메시지에서 오류가 발생한 위치 확인 | 확인함 | SELEC 부분에 문법 오류가 표시됨 | 수용 |
| SELEC를 SELECT로 수정 | 확인함 | 확인함 | 수용 |
| 현재 접속한 데이터베이스 재확인 | 확인함 | ad_database_book에 정상적으로 연결되어 있었음 | 수용 |

### AI가 오류 원인을 너무 빨리 단정한 부분이 있었나요?

```text
없었습니다. 오류 메시지에서 SELEC 부분을 명확하게 표시하고 있었기 때문에 AI가 제시한 철자 오류 가능성이 적절하다고 판단했습니다. 그래도 답변만 믿지 않고 SQL을 직접 수정하여 정상적으로 실행되는지 확인했습니다.
```

### 오류 메시지와 실제 환경 중 무엇을 확인해서 최종 판단했나요?

```text
오류 메시지에서 SELEC 근처에 문법 오류가 발생했다는 내용을 확인하고, 작성한 SQL을 올바른 SELECT 명령어와 비교했습니다. 수정 후 SELECT 1;이 정상적으로 실행되고 현재 데이터베이스도 ai_database_book으로 확인되어 문법 오류였다고 최종 판단했습니다.
```

### AI 활용에서 가장 유용했던 점

```text
오류 메시지의 의미를 쉽게 설명해 주고, 문제가 발생한 부분과 확인 순서를 빠르게 파악할 수 있도록 도와준 점이 가장 유용했습니다.
```

### AI 답변을 그대로 실행하지 않고 확인해야 하는 이유

```text
AI는 나의 실제 데이터베이스 환경을 모두 알지 못하며, 상황에 맞지 않거나 데이터에 영향을 주는 명령어를 제안할 수도 있기 때문이다. 따라서 오류 메시지와 현재 환경을 직접 확인하고, 제안된 방법이 안전한지 검토한 뒤 실행해야 한다.
```
...
사용할 데이터베이스 이름 후보: student_counseling

사용할 스키마 이름 후보:counseling

앞으로 만들고 싶은 테이블 후보 3개:
1. students: 상담을 신청하는 학생의 정보를 저장하는 테이블
2. consultations: 상담 내용, 상담 일시, 담당 강사 등의 정보를 저장하는 테이블
3. consultation_applications: 어떤 학생이 어떤 상담을 신청했는지와 신청 상태를 저장하는 테이블
```

### 아직 SQL을 만들지 않고 이름과 역할만 정하는 이유

```text
SQL을 작성하기 전에 학생 상담 서비스에 어떤 데이터가 필요하고 각 테이블이 어떤 역할을 담당할지 먼저 정해야 하기 때문이다. 구조를 먼저 정하면 데이터가 중복되거나 필요한 정보가 빠지는 문제를 줄일 수 있다.
```

### Chapter 02에서 정리했던 한 행의 의미 중 수정할 부분이 있나요?

```text
수정할 부분이 있다. 기존에는 세 번째 항목을 ‘상담 데이터 한 행’이라고 작성했지만, 실제로는 학생이 특정 상담을 신청한 사건 한 건을 의미하므로 ‘상담신청 데이터 한 행’으로 수정해야 한다.
```
...
```text
https://github.com/darajeong/ai-database-study/blob/main/assignments/chapter03/chapter03_answer.md
```
